...
export namespace CampaignGroundTypes {
  ...
  export class Entry {
    ...
    constructor(collectionCache: CollectionCache, values: any) {
      ...
      this._terrainAttritionMoveCursor = values["terrain_attrition_move_cursor"];
      this._terrainAttritionSelectCursor = values["terrain_attrition_select_cursor"];
    }
    ...
    get terrainAttritionMoveCursor(): Cursors.Entry | undefined {
      const collection = <Cursors.Entry[]>this.collectionCache.getCollection(Cursors.KEY, Cursors.Entry);
      return collection.find(entry => entry.key === this._terrainAttritionMoveCursor);
    }
    
    get terrainAttritionSelectCursor(): Cursors.Entry | undefined {
      const collection = <Cursors.Entry[]>this.collectionCache.getCollection(Cursors.KEY, Cursors.Entry);
      return collection.find(entry => entry.key === this._terrainAttritionSelectCursor);
    }
  }
}
