...
export namespace BattleEntityEffectsJunctions {
  ...
  export class Entry {
    ...
    constructor(collectionCache: CollectionCache, values: any) {
      ...
      this._bloodSpurtLargeDecal = values["blood_spurt_large_decal"];
      this._bloodExplosionDecal = values["blood_explosion_decal"];
      this._bloodSpurtLimb = values["blood_spurt_limb"];
    }
    ...
    get bloodSpurtLargeDecal(): ParticleEffects.Entry | undefined {
      const collection = <ParticleEffects.Entry[]>this.collectionCache.getCollection(ParticleEffects.KEY, ParticleEffects.Entry);
      return collection.find(entry => entry.key === this._bloodSpurtLargeDecal);
    }
    ...
    get bloodSpurtLimb(): ParticleEffects.Entry | undefined {
      const collection = <ParticleEffects.Entry[]>this.collectionCache.getCollection(ParticleEffects.KEY, ParticleEffects.Entry);
      return collection.find(entry => entry.key === this._bloodSpurtLimb);
    }
  }
}
