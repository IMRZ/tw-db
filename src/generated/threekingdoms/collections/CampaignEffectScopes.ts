...
export namespace CampaignEffectScopes {
  ...
  export class Entry {
    ...
    readonly _sourceForDesignRefOnly: string;
    readonly _target: string;
    readonly _ownership: string;
    readonly _location: string;
    readonly _territory: string;
    readonly sendToLocationOnlyIfAdult: boolean;
    readonly onlyDisplayInTooltip: boolean;

    constructor(collectionCache: CollectionCache, values: any) {
      this.collectionCache = collectionCache;
      this.key = values["key"];
      this.localisedText = values["localised_text"];
      this._sourceForDesignRefOnly = values["source_for_design_ref_only"];
      this._target = values["target"];
      this._ownership = values["ownership"];
      this._location = values["location"];
      this._territory = values["territory"];
      this.sendToLocationOnlyIfAdult = !!values["send_to_location_only_if_adult"];
      this.onlyDisplayInTooltip = !!values["only_display_in_tooltip"];
    }
    
    get sourceForDesignRefOnly(): CampaignEffectScopeObjects.Entry | undefined {
      ...
      return collection.find(entry => entry.key === this._sourceForDesignRefOnly);
    }
