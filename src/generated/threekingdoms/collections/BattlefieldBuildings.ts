...
export namespace BattlefieldBuildings {
  ...
  export class Entry {
    ...
    constructor(collectionCache: CollectionCache, values: any) {
      ...
      this._visibilityMode = values["visibility_mode"];
      this.compositeBuilding = !!values["composite_building"];
      this.metaTagGroups = values["meta_tag_groups"];
      this.allowSimpleMeshLodding = !!values["allow_simple_mesh_lodding"];
      this._audioBuildingDestruction = values["audio_building_destruction"];
      this.cullOnLowQuality = !!values["cull_on_low_quality"];
      this.cullOnMediumQuality = !!values["cull_on_medium_quality"];
    }
    ...
    get visibilityMode(): BattleBuildingVisibilityEnums.Entry | undefined {
      const collection = <BattleBuildingVisibilityEnums.Entry[]>this.collectionCache.getCollection(BattleBuildingVisibilityEnums.KEY, BattleBuildingVisibilityEnums.Entry);
      return collection.find(entry => entry.key === this._visibilityMode);
    }
    
    get audioBuildingDestruction(): AudioBuildingDestructionLevels.Entry | undefined {
      const collection = <AudioBuildingDestructionLevels.Entry[]>this.collectionCache.getCollection(AudioBuildingDestructionLevels.KEY, AudioBuildingDestructionLevels.Entry);
      return collection.find(entry => entry.key === this._audioBuildingDestruction);
    }
  }
}
